...
\begin{figure*}
    ...
    \subfigure[Spider feature]{%
        ...
        \hfill
    \subfigure[$c_0$]{%
        \includegraphics[width=\subwidth, clip=true, trim=130 105 300 30]{features_5}}
        \hfill
    \subfigure[$c_1$]{%
        \includegraphics[width=\subwidth, clip=true, trim=130 70 300 30]{features_6}}
        \hfill
    \subfigure[$c_2$]{%
        \includegraphics[width=\subwidth, clip=true, trim=130 70 300 30]{features_7}}
        \hfill
    \caption{
...
A Random Forest is a set of decision trees, each of which is trained on a different subset of the training data \cite{Breiman2001}. 
...
We use the entropy split criterion
\begin{equation}
- \sum_j p_j \log_2 p_j.
\end{equation}

Often the modal class vote is taken as a hard prediction of class membership. Instead we follow \cite{bostrom-mla-2007} in using the fraction of votes for each class to approximate a probability of class membership. $p_{ij}$ can be interpreted as the parameter of a categorical distribution.
%; \ie there is a probability $p_{ij}$ that $\imregion_i$ and $\imregion_j$ belong to the same class.
