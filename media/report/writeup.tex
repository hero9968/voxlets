...
\begin{itemize}
\item Box world --- predicting object locations and extents (and hence unseen completion) through bounding boxes. The problem with this is it gives incredibly coarse occupation information.
\end{itemize}
...
We use voxel carving instead, using out multiple rendered depth images. This gives more consistent results regardless of the mesh construction (figure \ref{fig:mesh_voxelisation_options}).
...
\begin{figure}
	...
	\figcaption{Depending on its construction, a CAD model (such as that of a box, top) may be voxelised in many different ways (top row). By using voxel carving (bottom row), we maintain consistency regardless of how the mesh is constructed, and the output is analogous to KinFU output.}%
