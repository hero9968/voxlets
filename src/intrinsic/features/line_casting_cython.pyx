...
def inner_loop(np.ndarray[double, ndim=2] input_im,
    ...
    '''
    given a start pixel and a direction, head in that direction until you run
    out of space on the input image

    This function modifies the output image(s), so returns None

    observed_indicator is an array of the same size as output_im, as stores
    whether or not the last voxel
...
def outer_loop(
        np.ndarray[double, ndim=2] input_im,
        np.ndarray[int, ndim=1] direction):
    '''
    given an input image and a direction in which to go, does all of the
    iterations
    '''
    ...
    if direction[0] > 0:
        ...
        for col in xrange(W):
            inner_loop(
                input_im, 0, col, direction, output_im, observed_to_be_full)

    elif direction[0] < 0:
        # start at all locations on the bottom
        for col in xrange(W):
            inner_loop(
                input_im, H - 1, col, direction, output_im,
                observed_to_be_full)
    ...
    if direction[1] > 0:
        ...
        for row in xrange(H):
            inner_loop(
                input_im, row, 0, direction, output_im, observed_to_be_full)

    elif direction[1] < 0:
        # start at all locations on right edge
        for row in xrange(H):
            inner_loop(
                input_im, row, W - 1, direction, output_im,
                observed_to_be_full)
...
def inner_loop_3d(
        np.ndarray[np.int8_t, ndim=3] input_im,
        int start_row,
        int start_col,
        int start_slice,
        np.ndarray[int, ndim=1] direction,
...
def outer_loop_3d(
    ...
    if direction[0] > 0:
        ...
        for col in xrange(W):
            for sslice in xrange(D):
                inner_loop_3d(
                    input_im, 0, col, sslice, direction, output_im,
                    observed_to_be_full)
        ...
        for col in xrange(W):
            for sslice in xrange(D):
                inner_loop_3d(
                    input_im, H - 1, col, sslice, direction, output_im,
                    observed_to_be_full)
    ...
    if direction[1] > 0:
        ...
        for row in xrange(H):
            for sslice in xrange(D):
                inner_loop_3d(input_im, row, 0, sslice, direction, output_im,
                              observed_to_be_full)
        ...
        for row in xrange(H):
            for sslice in xrange(D):
                inner_loop_3d(
                    input_im, row, W - 1, sslice, direction, output_im,
                    observed_to_be_full)
    ...
    if direction[2] > 0:
        ...
        for row in xrange(H):
            for col in xrange(W):
                inner_loop_3d(
                    input_im, row, col, 0, direction, output_im,
                    observed_to_be_full)
        ...
        for row in xrange(H):
            for col in xrange(W):
                inner_loop_3d(
                    input_im, row, col, D - 1, direction, output_im,
                    observed_to_be_full)
