...
def load_all_poses():
    ...
    return [pose.strip() for pose in open(filename, 'r') if pose[2] =='1']

poses = ['NP1_24']#load_all_poses()
endIdx = len(poses)
...
def load_from_ply(filename):
    ...
    faces = []
    for idx, face_data in enumerate(plydata['face'].data):
        #print face_data

        for face_datum in face_data[0]:
            t = list(np_vertex_data[int(face_datum)])
            #print face_datum
            #print face_datum
            faces.append(t)
        #print "newline"
    ...
    f = open('facefile.txt','w')
    for face in faces:
        f.write(str(face[0]) + " " + str(face[1]) + " " + str(face[2]) + "\n")
    f.close()

    print len(faces)
    print len(np_vertex_data)
    print faces[0]
    print np_vertex_data[0]
    #print faces[:100, :]
...
def loadXform():
    ...
    calib_h5 = h5py.File(bigbird_folder + modelname + "/calibration.h5", 'r')
    H_CAM_ir_from_NP5 = np.array(calib_h5['H_' + cameraname + '_ir_from_NP5'])

    # loading the transform from the mesh to np5
    np5_pose_filename = bigbird_folder + modelname + "/poses/NP5_" + pose_angle + "_pose.h5"
...
meshPath = "/Users/Michael/projects/shape_sharing/data/bigbird_meshes/" + modelname + "/meshes/poisson.ply"
#triangles, verts = load_from_ply("test_cinnamon.ply")
triangles, verts = loadOBJ("test_cinnamon.obj")

savePath =   "/Users/Michael/projects/shape_sharing/data/bigbird_renders/" + modelname + "/"
